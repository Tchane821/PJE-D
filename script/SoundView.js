...
export class SoundView extends NodePose3D { // intègre this.node placé par la pose du marqueur
    ...
    constructor() {
        ...
        for (let k = 0; k < nBar; k++) {
            ...
            this.node.add(cube);
            cube.position.copy(new THREE.Vector3(0 + k, 0, -2));
        }
    }
    ...
    update() {
        ...
        this.tool.equalizer; // aller chercher le tabl
        ...
        if(this.tool.dataArray !== undefined){
            for (let k = 0; k < 8; k++) {
                let values = this.dataArray[k * 2] + this.dataArray[k * 2 + 1]; // [0 -510]
                let transformedValue = (values / 100) + 0.5
                this.node[k].scale.set(transformedValue, 1, 1);
            }
        }
    }
}